...
```{r}
#package inca3 with the data on food consumption in France
...
Data are from a survey and [report by ANSES](https://www.anses.fr/fr/system/files/NUT2014SA0234Ra.pdf). All data and documentation are available on [data.gouv](https://www.data.gouv.fr/fr/datasets/donnees-de-consommations-et-habitudes-alimentaires-de-letude-inca-3/). Three files describe the data : [User guide](https://static.data.gouv.fr/resources/donnees-de-consommations-et-habitudes-alimentaires-de-letude-inca-3/20210128-192017/notice-utilisateurs-donnees-inca3-data.gouvjanv21.pdf) ([english](https://static.data.gouv.fr/resources/donnees-de-consommations-et-habitudes-alimentaires-de-letude-inca-3/20210128-192916/notice-utilisateurs-donnees-inca3-data.gouv-english.pdf)), [Nomenclature](https://static.data.gouv.fr/resources/donnees-de-consommations-et-habitudes-alimentaires-de-letude-inca-3/20210128-192229/nomenclature-inca3-janv2021.pdf), [Thesaurus](https://static.data.gouv.fr/resources/donnees-de-consommations-et-habitudes-alimentaires-de-letude-inca-3/20191217-185016/thesaurus-inca3-data.gouv.pdf)

Here are the data available in the package (in user guide document) :

not used

-   HABITUDES_INDIV, HABITUDES_MEN, ACTPHYS_SEDENT, FPQ : not used, describes individuals and households habits
-   CONSO_CA_PROD, CONSO_CA_INDIV : not used, tables about food supplement consumption.
-   OCCASIONS : not used (day of consumption, place...)
-   CONSO_COMPO_ALIM : very complete (250k obs, each ind, each aliment), not used yet but possible to study it
...
-   DESCRIPTION_INDIV : used for age and sex. Also includes data on weight, height, BMI...

-   APPORTS_NUT_ALIM : used, nutrients

## Loading data

We load our own files we produced with the wording of each categories.
...
File for each individual the nutrient intakes per day (40 000 observations). We only keep a few of them (K, P, protein, energy, carbohydrates, lipids)

```{r loading_data}
#nutriments for each individual OK
#individual ID, nutriments 1-54 (quantity/j), contribution 3-45 (%AESA, or for 100 kcal nutriment), 4114 obs (individuals)
glimpse(apports_nut_alim)
file_apports_nut_alim <- 
  apports_nut_alim %>% 
  # we only focus on individuals ID and nutrients
  select(-POPULATION) %>%
  gather(
    key = nutriment_contrib_nb, value = value_nutriment, nutriment1:contrib54
    ) %>%
  #we add the simplified nutrient names
  left_join(
    nutriments_contrib_INCA3, by="nutriment_contrib_nb"
    ) %>%
  filter(
    #only focus on energy, carbohydrates, lipids, proteins, phosphorus, and potassium
...
#simplified : only nutrients of interest + easier to handle
...
We adapt the individuals descriptions, with different age categories more or less precise

```{r}
file_individus <- description_indiv %>% 
  select(
    ID = NOIND, 
    sex = sex_PS, 
    age_class = tage_PS, 
...
file_individus$age_class <- 
  factor(file_individus$age_class, levels = c("<1", "1-3", "4-6", "7-10", "11-14", "15-17", "18-44", "45-64", "65-79"))

file_individus$age_class_large <- 
  factor(
    file_individus$age_class_large, 
    levels = 
      c(
...
file_individus <- file_individus %>% mutate(
...
We join the different nutrient intakes file and the individual description file.

```{r}
#joining tables
ID_intakes_day <- inner_join(file_individus, file_apports_nut_alim_simple, by = "ID") %>%
  rename(energy = AET)
```
...
```{r}
#nutrients by fine age and sex
temp <- 
  bind_rows(
  #summarise both sex, precise age categories
    ID_intakes_day %>% 
      select(
    ...
    #summarise with discrminated sex and precise age categories
    ID_intakes_day %>% 
      select(
        sex, age_class, age_median, 
        energy, proteins, lipids, carbohydrates, 
...
Here are presented macronutrients intakes by age and sex. For the relative macronutrients shares in energy intake, the results are very similar whatever the sex or the age.
...
We only show the detailed categories table, because an unweighed mean would not consider the age distribution of the population and would give excessive weight to young population (which are smaller). This issues is addressed in the next section "Nutrient excretion in France".
...
```{r table_sex_age_rough, results = T}
...
```{r table_sex_age_detailed, results = T}
#nutrients x sex x detailes age category
temp <- bind_rows(
  ID_intakes_day %>% 
    select(
      sex, age_class, 
...
We prepare the population data by cities and age to be transformed into nutrient excretions by cities.
...
#give explicit sex and age names
population_age_commune <- 
...
#creating same file for department and region scale
#department
...
For the water agencies basins, we join the commune data with our key file linking cities to water agencies basins.
...
We load the excretion data by age, and combine it with population data to have the quantities excreted in France, by basin, by region, by department and by commune.

```{r nutrient_excretion_loading}
...
rm(adult_excretion, children_excretion, all_excretion)
```

```{r}
f_graph_map_excretion <- function(dataset, nutrient, nutrient_label){
  g <- ggplot(dataset) +
    geom_sf(aes(fill = !!as.symbol(nutrient)), color = "black") + 
    coord_sf(datum = NA, expand = FALSE) + 
    geom_sf_text(aes(label = !!as.symbol(nutrient))) +
    labs(
      title = paste("Human", nutrient_label, "excretions"),
      fill = paste0("kt", nutrient_label, " per year"), 
      x="", y="",
      caption = Source
      ) 
  return(g)
...
temp2 <- excretion_france %>%
  ...
  mutate(
    quantity = case_when(
      ...
      quantity == "capita" ~ paste("population", temp$capita, "million"),
    )
  )
...
We compare the basin population we estimated with values reported by the different water agencies. There is a rather good matching. We apply a coefficient to our computed excretions based on our estimation of population values to match the population figures reported by the water agencies.
